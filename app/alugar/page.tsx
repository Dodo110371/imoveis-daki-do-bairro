...
interface AlugarPageProps {
  searchParams: Promise<AlugarSearchParams>;
}

type AlugarDbProperty = {
  id: string;
  title: string;
  price: number;
  type: "Venda" | "Aluguel";
...
export default async function AlugarPage({ searchParams }: AlugarPageProps) {
  ...
  const agencyIds: number[] = Array.from(
    new Set((propertiesData || [])
      .map((p: AlugarDbProperty) => p.agency_id)
  ...
  const ownerIds: string[] = Array.from(
    new Set((propertiesData || [])
      .map((p: AlugarDbProperty) => p.owner_id)
  ...
  const properties = (propertiesData || []).map((p: AlugarDbProperty) => {
